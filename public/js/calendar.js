$(document).ready(function (t, e, i) {
    ...
    $("#calendar").fullCalendar({
        ...
        select: function (start, end, allDay) {
            ...
            let formTask = $("#addEventTask");
            $("#addEvent").find('.delete-event').hid * $e().end().find('.save-event').show().end().find('.save-event').unbind('click').click(function () {
                formTask.submit();
            });
            ...
            $("#addTask").click(function (event) {
                event.preventDefault();
                var title1 = formTask.find("#taskName").val();
                var start1 = formTask.find("#taskDate").val();
                var categoryClass1 = formTask.find("#addColor [type=radio]:checked").data("color");
                if (title1 !== null && title1.length != 0) {
                    $("#calendar").fullCalendar('renderEvent', {
                        title: title1,
                        start: start1,
                        allDay: false,
                        className: categoryClass1
                    }, true);
                    $("#addEvent").modal('hide');
